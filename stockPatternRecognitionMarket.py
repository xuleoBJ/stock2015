...
def printResult(curStock, kMatchIndexList):
    ...
    value_smaller_1 = 0
    value_bigger_1 = 0
    for iValue in riseRateNextList:
        if iValue < 0:
            value_smaller_0 = value_smaller_0+1
        if iValue < -1:
            value_smaller_1 = value_smaller_1+1
        if iValue >= 1:
            value_bigger_1 = value_bigger_1 + 1
    if matchNum > 0:
        ...
        lineWritedList.append(u"{:2d}   \t{:2d}({:.2f}%)\t{}\t{}".format(
            matchNum, value_smaller_0, float(value_smaller_0)*100/matchNum, \
                value_bigger_1, value_smaller_1))
...
#        lineWritedList.append(u"最低涨幅:{}\t中位数:{:.2f}".format(valueLowestLine,_median))
    ...
    lineWritedList.append(
        u"日期[星期]\t次日涨幅\t次日开盘\t次日最高\t次日最低\t量能\t当日涨幅\t当日最高\t当日最低\t当日波动幅度\t60日周期(%)\t20日周期(%)\t10日周期(%)\t5日周期(%)")
    for index in kMatchIndexList:
        ...
        resultLine = u"{0:<10}[{1}]\t{2}\t{3}\t{4}\t{5}\t{6}\t{7}\t{8}\t{9}\t{10}\t{11}\t{12}\t{13}\t{14}".format(curStock.dayStrList[index], curStock.weekDayList[index],
                                                                                                                  curStock.dayRiseRateCloseFList[
                                                                                                                      index+1], curStock.dayRiseRateOpenFList[index+1],
                                                                                                                  curStock.dayRiseRateHighestArray[
                                                                                                                      index+1], curStock.dayRiseRateLowestArray[index+1],
                                                                                                                  curStock.dayRadioLinkOfTradeVolumeFList[
                                                                                                                      index], curStock.dayRiseRateCloseArray[index],
                                                                                                                  curStock.dayRiseRateHighestArray[index], curStock.dayRiseRateLowestArray[
                                                                                                                      index], curStock.dayWaveRateArray[index],
                                                                                                                  getRefVrise(
                                                                                                                      curStock, index, 60),
                                                                                                                  getRefVrise(
                                                                                                                      curStock, index, 20),
...
def patternRecByMarketAndStock(curMarket, curStock, matchDateIndex):
    ...
    for i in range(-500, -1):
        ...
        while iCount <= 2 and bSelect == True:
            # 考虑大盘
            valueRate = math.floor(
                curMarket.dayRiseRateCloseFList[matchDateIndex-iCount]/bias)*bias
        ...
        while iCount <= 2 and bSelect == True:
            # 考虑个股
            valueRate = math.floor(
                curStock.dayRiseRateCloseFList[matchDateIndex-iCount]/bias)*bias
...
def patternRecByPriceOpen(curStock, matchDateIndex, kMatchIndexList, bias=0.3):
    ...
    for index in kMatchIndexList:
        iCount = 0
        valueRate = math.floor(
            curStock.dayPriceOpenFList[matchDateIndex-iCount]/bias)*bias
...
def patternRecByRiseWave(curStock, matchDateIndex, kMatchIndexList, bias=0.5):
    ...
    for index in kMatchIndexList:
        iCount = 0
        valueRate = math.floor(
            curStock.dayWaveRateFList[matchDateIndex-iCount]/bias)*bias
#	    print valueRate
...
def patterRecByHandSet(curStock, iTradeDay, kNum):
    ...
    for i in range(-iTradeDay+2, -1):
        ...
        if riseRate_i_1 <= curStock.dayRiseRateCloseFList[i-1] <= 1+riseRate_i_1 and riseRate_i <= curStock.dayRiseRateCloseFList[i] <= 1+riseRate_i:
            ...
            bSelect = True
        if bSelect == True:
            weekDay = Ccomfunc.convertDateStr2Date(
                curStock.dayStrList[i]).isoweekday()
            print(curStock.dayStrList[i], "weekDay_"+str(weekDay),
                  "RiseRateofNextTradeDay: "+str(curStock.dayRiseRateCloseFList[i+1]))
            print(u"{},星期{},次日涨幅:{}".format(
                curStock.dayStrList[i], weekDay, curStock.dayRiseRateCloseFList[i+1]))
            print("_"*30+"riseRate", curStock.dayRiseRateCloseFList[i-2],
                  curStock.dayRiseRateCloseFList[i-1], curStock.dayRiseRateCloseFList[i])
            print("_"*30+"turnOverRate=", curStock.dayRiseOfTurnOverFList[i-2],
                  curStock.dayRiseOfTurnOverFList[i-1], curStock.dayRiseOfTurnOverFList[i-1])
...
def calMoodIndexFromRecogitionPattern(curStock, iTradeDay, kNum, matchDateIndex, bias):
    kPatternListYestoday = patternRecByRiseRate(
        curStock, iTradeDay, kNum, matchDateIndex-1, bias)
    # 识别结果统计分析
    dateList = []
    riseRateNextList = []
    ...
    # 计算量能，根据量能修正昨日市场情绪参数
    volEnergeRate = float(
        curStock.dayTradeVolumeFList[matchDateIndex]) / curStock.dayTradeVolumeFList[matchDateIndex-1]
...
def calMarketRiskIndex(curStock, moodIndex, kMatchIndexList):
    ...
    if 1.5 <= marketRiskIndex <= 3.5:
        marketRiskIndex = marketRiskIndex - \
            (value_bigger_1 - value_smaller_1)*0.2
...
def calResistLine(curStock, strDate):
    ...
    for period in [5, 10, 20, 30, 60, 90, 120, 150, 180]:
        cycleHigh = curStock.dayPriceHighestArray[matchDateIndex -
                                                  period:matchDateIndex+1].max()
        cycleLow = curStock.dayPriceLowestArray[matchDateIndex -
                                                period:matchDateIndex+1].min()
        for keyPoint in [0.25, 0.33, 0.5, 0.618, 0.825]:
            ...
            if resistLinePoint != 0:
                calRiseRate = 100 * \
                    (resistLinePoint -
                     curStock.dayPriceClosedArray[-1])/resistLinePoint
            resultLine = u"{}日\t{}\t{}\t{}\t{:.2f}\t{:.2f}%".format(
                period, keyPoint, cycleLow, cycleHigh, resistLinePoint, calRiseRate)
...
def getRefVrise(curStock, indexOfdate, dayOfCycle):
    dayMaxClose = max(
        curStock.dayPriceClosedArray[indexOfdate-dayOfCycle:indexOfdate+1])
    dayMinClose = min(
        curStock.dayPriceClosedArray[indexOfdate-dayOfCycle:indexOfdate+1])
    dayBiaoZhun = dayMaxClose

    upRise = str(round(
        100*(curStock.dayPriceClosedArray[indexOfdate]-dayMinClose)/dayMinClose, 2))
    downRise = str(round(
        100*(curStock.dayPriceClosedArray[indexOfdate]-dayMaxClose)/dayMaxClose, 2))
...
def recogitionPatternByDateIndex(curStock, matchDateIndex):
    ...
    lineWritedList.append(
        u"日期[星期]    \t价格\t涨幅\t最大涨幅\t最小涨幅\t量比\t波动幅度\t60日涨幅%\t20涨幅%\t10涨幅%\t5涨幅%")
    for i in range(matchDateIndex+1-kNum, matchDateIndex+1):  # 循环指数起始比匹配指数少1
        ...
        resultLine = u"{}[{}]\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}".format(curStock.dayStrList[i], weekDay, curStock.dayPriceClosedArray[i], curStock.dayRiseRateCloseFList[i],
                                                                              curStock.dayRiseRateHighestArray[
                                                                                  i], curStock.dayRiseRateLowestFList[i],
                                                                              curStock.dayRadioLinkOfTradeVolumeFList[
                                                                                  i], curStock.dayWaveRateFList[i],
                                                                              getRefVrise(
                                                                                  curStock, i, 60),
                                                                              getRefVrise(
                                                                                  curStock, i, 20),
    ...
    print("-"*72)
    kPatternList = patternRecByRiseRate(
        curStock, iTradeDay, kNum, matchDateIndex, bias)
   ...
   # patternRecByRiseWave(curStock,matchDateIndex,kPatternList)
    ...
    print("-"*72)
    print(u"计算市场情绪指数")
    moodIndex = calMoodIndexFromRecogitionPattern(
        curStock, iTradeDay, kNum, matchDateIndex, bias)
...
def mainAppCall(strDate=""):
    ...
    dirJGY = "D:\\我的坚果云\\3_patternRecDir\\"
