...
def findPeakPrice(dayPeriod, curDateStrList, curPriceOpenFList, curPriceHighestFList, curPriceLowestFList, curPriceClosedFList, resultDir="resultDir"):
    ...
    goalFilePath = os.path.join(
        resultDir, stockID+"_"+str(dayPeriod)+'_peakAnalysisPrice.txt')  # 输出文件名
    lineWritedList = []

    lineWritedList.append('-'*50)
    lineWritedList.append('价格峰值分析分析周期(天):'+str(dayPeriod))
    lineWritedList.append("日期"+"\t距上次峰值交易日个数\t" +
                          "\t距上次峰值自然日个数\t"+"\t局部高点/低点\t"+"浮动幅度%:\t")
    ...
    indexLast = 1
    dayPeriod = int(dayPeriod/2)  # 周期内最值 用半周期前后算，i循环时 比较当日是否是前后半周期的极值
...
def findPeakVolume(dayPeriod, curDateStrList, curTradeVolumeFList):
    ...
    goalFilePath = os.path.join(
        resultDir, stockID+"_"+str(dayPeriod)+'_peakAnalysisVolume.txt')  # 输出文件名
    lineWritedList = []
    lineWritedList.append('-'*50)
    lineWritedList.append('成交量峰值分析周期(天):'+str(dayPeriod))
    lineWritedList.append("日期"+"\t局部高点/低点(万手)\t" +
                          "\t距上次峰值交易日个数\t"+"\t距上次峰值自然日个数\t"+"\t浮动幅度%:\t")
...
def findPeakTurnover(dayPeriod, curDateStrList, curTurnover):
    ...
    lineWritedList.append("日期"+"\t局部高点/低点(亿元)\t" +
                          "\t距上次峰值交易日个数\t"+"\t距上次峰值自然日个数\t"+"\t浮动幅度%:\t")
    d1 = Ccomfunc.convertDateStr2Date(curDateStrList[0])
    d2 = Ccomfunc.convertDateStr2Date(curDateStrList[0])
    standValue = 100
...
def analysisDate(dateStrStart, dateStrEnd, curDateStrList, curPriceOpenFList, curPriceHighestFList, curPriceLowestFList, curPriceClosedFList):
    ...
    print("-"*50)
    print("分析周期(交易日/天):\t"+str(indexEnd-indexStart)+"起始日期:\t" +
          curDateStrList[indexStart]+"\t结束日期:"+curDateStrList[indexEnd])
    ...
    print("最高点出现与最低点出现交易日个数(天):\t"+str(1+curPriceHighestFList.index(curPriceHighest) -
