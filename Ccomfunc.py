...
##计算最后一个交易日前，interValDay个交易日的比今天的涨幅，interValDay是间隔数，-5就是交易日的前5天与今天的涨幅，+3 就是三日后比今天的涨幅，
def calTrend(curStock,intervalDay):
    indexLastOne=len(curStock.dayPriceClosedFList)-1
    if intervalDay<0 and curStock.dayPriceClosedFList[indexLastOne+intervalDay]>0:
        return calRiseRateInterval(curStock,indexLastOne,intervalDay) 
    else:
		return -999
##输出交易日的差额
...
def printInfor():
    ctypes.windll.user32.MessageBoxA(0, "Some money is not in my system.", "infor", 1)
    print("\n"+"#"*80)
    print(u"1.股市需要耐心，股市态度要认真,有的钱不去赚。")
