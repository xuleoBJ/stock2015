...
def UpDownStastic(cStock, indexStart, indexStartEnd):
    ...
    for i in range(indexStart, indexStartEnd+1):
        deltaCloseHigh = cStock.dayRiseRateCloseArray[i] - \
            cStock.dayRiseRateHighestArray[i]
        deltaLowOpen = cStock.dayRiseRateLowestArray[i] - \
            cStock.dayRiseRateOpenArray[i]
        ...
        deltaHighOpen = cStock.dayRiseRateHighestArray[i] - \
            cStock.dayRiseRateOpenArray[i]
        deltaCloseLow = cStock.dayRiseRateCloseArray[i] - \
            cStock.dayRiseRateLowestArray[i]
...
def printCalTrend(curStock, intervalDay):
    print(str(intervalDay)+u"个交易日日累计涨幅:" +
          str(round(calTrend(curStock, intervalDay), 2))+"%")
...
if __name__ == "__main__":
    ...
    lineWritedList.append(
        "年度"+"\t"+"0101-"+str(int(strMonth)-1)+strDayEnd+"涨幅"+"\t"+strMonth+"月涨幅")
# 按月统计涨幅 振幅 最大波动幅度，最小波动幅度
    for year in range(2000, 2017):
        dateYearFirstDate = str(year)+"/"+"01"+"/"+"01"
        indexOfYearFirstDate = Ccomfunc.getIndexByStrDate(
            curStock, dateYearFirstDate)
        ...
        riseOfYearFirstDay2monthFirstDay = calRiseRateClosed(
            curStock, indexOfYearFirstDate, indexOfStartDate-1)
        riseRateMonth = calRiseRateClosed(
            curStock, indexOfStartDate, indexOfDateEnd)
        lineOut = u"{}年\t{:.2f}\t{:.2f}".format(
            year, riseOfYearFirstDay2monthFirstDay, riseRateMonth)
